...
export class PayloadBuilder {
  ...
  async build(files: FileMetadata[]): Promise<PurviewPayload[]> {
    const conversationId = this.generateConversationId();
    const allMessages: PurviewMessage[] = [];
    ...
    const payloads: PurviewPayload[] = [];
    let currentMessages: PurviewMessage[] = [];
    let currentSize = 0;
  ...
  private createPayloadObject(conversationId: string, messages: PurviewMessage[], fileCount: number): PurviewPayload {
    return {
      conversationId,
      messages,
      metadata: {
  ...
  private createMetadataMessage(files: FileMetadata[]): PurviewMessage {
    const summary = {
  ...
  private async createFileMessages(file: FileMetadata): Promise<PurviewMessage[]> {
    const messages: PurviewMessage[] = [];
  ...
  private chunkContent(content: string, chunkSize: number = 50000): string[] {
    const chunks: string[] = [];
  ...
  private getFileTypes(files: FileMetadata[]): Record<string, number> {
