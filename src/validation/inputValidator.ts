...
export async function validateInputs(): Promise<ActionConfig> {
  ...
  try {
    ...
    let userId: string;
    let userMappings: import('../config/types').UserMapping[] | undefined;
    ...
    if (isExternalWorkflowRepo && apiTokenForUsersJson) {
      ...
      try {
        ...
        if (Array.isArray(data) || !('content' in data)) {
          throw new Error(
            `${usersJsonPath} in ${workflowRepo!.owner}/${workflowRepo!.repo} is not a file.`
          );
        }

        const content = Buffer.from(data.content, 'base64').toString('utf-8');
        parsed = JSON.parse(content) as UsersConfig;
        logger.info(
      ...
      if (!fs.default.existsSync(absPath)) {
        throw new Error(
          `users.json not found at ${absPath}. ` +
          'Create a users.json in your workflow-definition repo with email-to-userId mappings and a defaultUserId.'
        );
      }

      parsed = JSON.parse(
        fs.default.readFileSync(absPath, 'utf-8')
      ) as UsersConfig;
    ...
    const repository: ActionConfig['repository'] = {
...
function validateClientCertificatePem(pem: string): void {
